...
def prep_scement_intersect(ad_objects):
...
def run_avalve_dsets1(data_dir, output_dir):
    ...
    cp_axcat = prep_scement_intersect(full_ads)
    sc._settings.ScanpyConfig.figdir = pathlib.Path(output_dir)
    cp_axcat_plot_file = "scement_avalve.png"
    cp_axcat_results_file = output_dir + "/" + "scement_avalve.h5ad"
    ...
    #
    tic = time.perf_counter()
    scement.sct_dense(cp_axcat, key="batch", inplace=True)
    ...
    toc = time.perf_counter()
    print(f"SCEMENT in {toc - tic:0.4f} seconds")
    #
    cp_axcat_filter_plot_file = "scement_avalve_filter.png"
    cp_axcat = cp_axcat[cp_axcat.obs["cell_type"] != "Unknown"]
    sc.pl.umap(cp_axcat, color=["batch", "cell_type"], save=cp_axcat_filter_plot_file)
    cp_axcat_sfilter_plot_file = "scement_avalve_sfilter.png"
    ...
    cp_axcat2.obs["batch"] = cp_axcat2.obs[
        "batch"
    ].cat.rename_categories(  # type:ignore
        {"H1": "Healthy-1", "H2": "Healthy-2", "C3": "Diseased-1", "C4": "Diseased-2"}
    )
    cp_axcat2.obs["cell_stype"] = cp_axcat2.obs[
        "cell_stype"
    ].cat.rename_categories(  # type:ignore
        {
    ...
    cp_axcat_sfilter2_plot_file = "scement_avalve_final.png"
    sc.pl.umap(
        cp_axcat2, color=["batch", "cell_stype"], save=cp_axcat_sfilter2_plot_file
    )
