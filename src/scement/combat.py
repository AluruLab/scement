from typing import Collection, Sequence, Tuple, Optional, Union
import pandas as pd
import numpy as np
...
def _design_matrix(
    ...
    design[design.select_dtypes(np.float64).columns] = design.select_dtypes(
...
def _standardize_data(
    ...
    design = _design_matrix(model, batch_key, batch_levels, data.values.dtype)

    # compute pooled variance estimator
    B_hat = np.dot(np.dot(np.linalg.pinv(np.dot(design.T, design)),
                          design.T),
                   data.T)
...
def combat(
    ...
    parameter_shift : Optional[dict[str, float]] = None,
    ...
    if issparse(adata.X):
        X = adata.X.A.T.astype(run_dtype)  # type:ignore
    else:
    ...
    model : pd.DataFrame = adata.obs[[key] + [covariates if covariates else []]]  # type: ignore
    ...
    gamma_hat = (
        np.linalg.pinv(batch_design.T @ batch_design) @ batch_design.T @ s_data.T
    ).values  # type:ignore
    ...
    for i, batch_idxs in enumerate(batch_info):
        ...
        pgbar, pt2, pa, pb = gamma_bar[i], t2[i], a_prior[i], b_prior[i]
        if parameter_shift is not None:
            if 'gbar' in parameter_shift:
                pgbar = pgbar + pgbar * parameter_shift['gbar']
            if 't2' in parameter_shift:
                pt2 = pt2 + pgbar * parameter_shift['t2']
            if 'a' in parameter_shift:
                pa = pa + pa * parameter_shift['a']
            if 'b' in parameter_shift:
                pb = pb + pb * parameter_shift['b']
        print(f"PRIORS for {i} : {pgbar}, {pt2}, {pa}, {pb}")
        gamma, delta = _it_sol(
    ...
    for j, batch_idxs in enumerate(batch_info):
        ...
        numer = np.array(
            bayesdata.iloc[:, batch_idxs] - np.dot(batch_design.iloc[batch_idxs], gamma_star).T
        )
